...
public class RunningThreadContainer extends MultiElementContainer<CancelScreen> {
    ...
    public RunningThreadContainer(int x, int y, int width, int height, FontRenderer textRenderer, CancelScreen parent, ScriptThreadWrapper t) {
    ...
    public void render(int mouseX, int mouseY, float delta) {
        try {
            if (t.t != null && t.t.isAlive()) {
                if (this.visible) {
                    drawCenteredString(textRenderer, textRenderer.trimStringToWidth(t.t.getName(), width - 105 - height), x + (width - 105 - height) / 2 + height + 4, y+2, 0xFFFFFF);
