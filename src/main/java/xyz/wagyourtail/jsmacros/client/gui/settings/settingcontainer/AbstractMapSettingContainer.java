        ...
        addButton(new Button(x, y, 40, 10, textRenderer, 0, 0xFF000000, 0x7FFFFFFF, 0xFFFFFF, new ChatComponentTranslation("jsmacros.add"), (btn) -> {
            if (setting.hasOptions()) {
                try {
                    ...
                    openOverlay(new SelectorDropdownOverlay(x, y + 10, width / 2, options.size() * textRenderer.FONT_HEIGHT + 4, options.stream().map(ChatComponentText::new).collect(Collectors.toList()), textRenderer, getFirstOverlayParent(), (i) -> newField(options.get(i))));
